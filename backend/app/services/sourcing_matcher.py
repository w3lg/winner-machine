...
class SourcingMatcher:
    ...
    def find_sourcing_options_for_candidate(
        ...
        # Si aucune option trouvée, créer une option auto-générée
        if not options:
            logger.info(
                f"Aucun match trouvé pour {candidate.asin}, création d'une option de sourcing auto-générée"
            )
            auto_option = self._create_default_sourcing_option(candidate)
            if auto_option:
                options.append(auto_option)
                logger.debug(
                    f"Option auto-générée créée pour {candidate.asin}: "
                    f"supplier={auto_option.supplier_name}, cost={auto_option.unit_cost}"
                )
    ...
    def _create_default_sourcing_option(
        ...
        """
        Crée une option de sourcing auto-générée pour un produit sans match.

        Cette option utilise des valeurs estimées basées sur le produit lui-même.
        Si aucun match n'est trouvé dans les catalogues CSV, cette méthode garantit
        qu'au moins une option de sourcing sera disponible pour le scoring.

        Args:
            candidate: Produit candidat.

        Returns:
            Instance SourcingOption auto-générée (toujours retournée).
        """
        # Calculer un prix d'achat estimé : 30% du prix de vente moyen
        # Si avg_price est None, utiliser 10.0 par défaut
        if candidate.avg_price:
            estimated_unit_cost = Decimal(str(float(candidate.avg_price) * 0.30)).quantize(
                Decimal("0.01")
            )
        else:
            estimated_unit_cost = Decimal("10.0")

        # Créer l'option auto-générée selon les spécifications
        return SourcingOption(
            product_candidate_id=candidate.id,
            supplier_name="AutoGenerated Supplier",
            sourcing_type="auto",
            unit_cost=estimated_unit_cost,
            shipping_cost_unit=Decimal("2.0"),
            moq=1,
            lead_time_days=3,
            brandable=False,
