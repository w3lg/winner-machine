#!/usr/bin/env python3
"""
Script pour créer le workflow n8n "WM Winners → Listings Drafts"
"""
import json
import requests
import sys
...
def create_listing_workflow():
    """Crée le workflow n8n pour générer les listings"""
    ...
    print("📖 Lecture du fichier workflow listings...")
    try:
        with open("n8n/workflows/wm_winners_to_listings_drafts.json", "r", encoding="utf-8") as f:
    ...
    workflow_payload = {
        "name": workflow_data.get("name", "WM Winners → Listings Drafts"),
    ...
    # 4. Créer le workflow
    print("🚀 Création du workflow listings...")
...
if __name__ == "__main__":
    print("=" * 60)
    print("Création du workflow n8n Winners → Listings Drafts")
    print("=" * 60)
    print()
    
    workflow_id, workflow_name = create_listing_workflow()
    
    print()
    print("=" * 60)
    if workflow_id:
        print("✅ Le workflow listings a été créé avec succès!")
        print(f"   Nom: {workflow_name}")
        print(f"   ID: {workflow_id}")
        print(f"   Statut: ACTIF")
        print(f"   Planification: Tous les jours à 04:00 (cron: 0 4 * * *)")
    else:
